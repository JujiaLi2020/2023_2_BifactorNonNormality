...
main.folder <- "C:/Users/julia/OneDrive - The University of Alabama/Research/BifactorNonNormality"
...
plot <- ggplot(data = df, aes (x = Fg, y = value, group = Sample_Size))+
  geom_line(aes(linetype = Sample_Size)) +
  scale_linetype_manual(values = c("dotted", "dashed", "F1",  "solid")) +
  facet_grid(Factor~I)+
...
plot <- ggplot(data = df, aes (x = Fs, y = value, group = Sample_Size))+
  geom_line(aes(linetype = Sample_Size)) +
  scale_linetype_manual(values = c("dotted", "dashed", "F1",  "solid")) +
  facet_grid(Factor~I)+
...
plot <- ggplot(data = df, aes (x = Fg, y = value, group = Sample_Size))+
  ...
  labs(x = "Distribution of Ability (Theta) on the General Factor", y = "Bias", size = 1.5)+
...
plot <- ggplot(data = df, aes (x = Fg, y = value, group = Sample_Size))+
  ...
  labs(x = "Distribution of Ability (Theta) on the General Factor", y = "RMSE", size = 1.5)+
  theme(axis.text.x=element_text(angle=0, vjust=0,hjust=0.5, size = 12),
        plot.title = element_text(hjust = 0.5))+
  geom_hline(yintercept = 0, linetype='dashed', col = 'black')
plot
